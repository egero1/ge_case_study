...
The 'caret' package in R provides the 'rfe' function for recursive feature elimination which is a simple backwards selection process to identify the best predictors. The process works by including all variables in the model and removing variables one at a time to test the accuracy. If the accuracy improves, another variable is removed and the model is tested again. Once all combinations of variables have been tested, the process completes. __Figure 6__ shows that the best accuracy - roughly 93%, is achieved with 16 variables. Note that the accuracy degrades as more predictors are added to the model. 
