...
model_data <- use_data %>%
        select(predictors(pref_variables), Label)
...
glm.cm <- caret::confusionMatrix(glm.model$pred$pred, model_data$Label, mode = "everything")
...
glm.ROC <- roc(model_data$Label, glm.model$pred$Normal)
