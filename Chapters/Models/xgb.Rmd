...
Gradient Boosting is a machine learning technique that can be used for regression and classification. It works by creating an ensemble of weaker prediction models in a stage-wise fashion then generalizes them through the optimization of an arbitrary differentiable loss function.
...
The confusion matrix in __Table 20__ shows that the model incorrectly predicts Pneumoconiosis in 20 patients when the disease is not actually present and fails to predict Pneumoconiosis in only 29 patients when the disease actaully is present.
