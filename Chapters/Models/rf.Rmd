...
The Random Forest algoritm is also quite popular and can be used for regression or classification. They are known for above average results since they encompass all the benefits of decision trees, and they are not prone to over-fitting. 
...
As previosly noted, AUC in __Figure 15__ of `r rf.auc` is slightly misleading. While the specificity is quite good, the sensitivety still shows some issues. If one were to evaluate the model based on soley on the AUC, they might believe the model is performing much better than it actually is.
